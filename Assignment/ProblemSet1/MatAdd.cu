...
__global__ void MatAdd(const float*, const float*, float*, int, int);
...
int main(){
	...
	// temp				 -> For temperory store the data
	// 
    ...
    long size = N * M * sizeof(float);

    // Allocate matrix A, B, C on host memory
    ...
    // Allocate the matrices in device memory
